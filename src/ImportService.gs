function getHonobonoImportRows_(targetMonth, importBatch) {
...
function getEshuImportRows_(targetMonth, importBatch) {
  var month = normalizeYearMonth_(targetMonth);
...
function getLatestHonobonoMap_(targetMonth) {
  var batch = getLatestImportBatch_(targetMonth, APP.SHEETS.HONOBONO);
  if (!batch) return { batch: 0, rows: {}, list: [] };
  var rows = getHonobonoImportRows_(targetMonth, batch);
  var map = {};
  ...
  return { batch: batch, rows: map, list: rows };
}

function getLatestEshuMap_(targetMonth) {
  var batch = getLatestImportBatch_(targetMonth, APP.SHEETS.ESHU);
  if (!batch) return { batch: 0, rows: {}, list: [] };
  var rows = getEshuImportRows_(targetMonth, batch);
  var map = {};
  rows.forEach(function(row) {
    var matchId = normalizeIdForMatch_(row['照合用ID']);
    if (!matchId) return;
...
function buildImportRow_(type, targetMonth, batch, lineNo, rawId, users, amountResult, rawName) {
  ...
  if (!matchId) {
    ...
    judgment = APP.IMPORT_JUDGMENT.ID_UNREGISTERED;
  ...
  return Object.assign(common, {
    ...
    '請求金額': amountResult.ok || amountResult.warning ? amountResult.value : ''
  });
}
